...
class Profiler():
	...
	def process_task(workgroup_id, task_id, app_root, app_output_file, app_runtime_pattern):
	    app_runtime = None
	
	    # Construct the path to the app output file
	    app_outputfile_path = os.path.join(app_root, f'A{task_id}/' + app_output_file)
	
	    # Check if the app output file exists
	    if os.path.exists(app_outputfile_path):
	        try:
	            with open(app_outputfile_path, 'r') as f:
	                for line in f:
	                    # Find the CPU runtime pattern
	                    if app_runtime_pattern in line:
	                        ls = line.split()
	                        app_runtime = float(ls[3])  # Convert to seconds
	                        break  # Stop searching once found
	        except Exception as e:
	            print(f"Error processing task {task_id}: {e}")
	
	    return workgroup_id, task_id, app_runtime


	# Define the parallel processing function
	def parallel_process_workgroup(workgroup_id, app_root, app_output_file, app_runtime_pattern):
	    tasks = self.master_log_json[workgroup_id].keys()
	    ...
	    with ProcessPoolExecutor() as executor:
	        ...
	        for task_id in tasks:
	            future = executor.submit(self.process_task, workgroup_id, task_id, app_root, app_output_file, app_runtime_pattern)
	            futures.append(future)
	
	        results = []
	
	        # Asynchronously collect results from completed tasks
	        for future in tqdm(futures, desc=f"Processing Workgroup {workgroup_id}"):
	            try:
	        ...
	        # Update the master_log_json with the processed results
	        for result in results:
	            workgroup_id, task_id, app_runtime = result
	...
	def update_app_runtime(self,app,root):
		...
		if app == 'gulp':
			...
			if os.path.exists(root):
				...
				# Loop over workgroup_ids and process each workgroup in parallel
				with ProcessPoolExecutor() as executor:
				    for workgroup_id in self.master_log_json.keys():
				        executor.submit(self.parallel_process_workgroup, workgroup_id, self.app_root, _app_output_file, _app_runtime_pattern)

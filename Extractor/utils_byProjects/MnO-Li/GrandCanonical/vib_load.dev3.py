...
def get_gz(csvdf,T,vib=False,pkldf=None):
	...
	#
		...
		for E,taskid in zip(Elist,csvdf['taskid'].values):
			...
			'''
				loop thruough modes : freq
			'''
			for freq in freqlist:

				freq = np.float128(freq)
				freq0_power += (0.5 * freq * _wavenumber_to_ev)					# ZPE contribution
				freq_infsum *= (1./(1.-np.exp(-freq*_wavenumber_to_ev/_kb/T)))	# quantum number sum
				#print((1./(1.-np.exp(-freq * _wavenumber_to_ev))))

			Zzpe = np.exp(-freq0_power/_kb/T)
			Zinfsum = freq_infsum
			...
			# sum up
			#Z += (ZE * Zzpe)	# depreecated for ZE & ZPE
			Z += (ZE * Zzpe * Zinfsum)

		# pull back Egm
		Z = np.exp(-Egm/_kb/T) * Z
		G = - _kb * T * np.log(Z)	# possible division 0 Error

		print('G,Z,ZE,Zzpe,Zinfsum',G,Z,ZE,Zzpe,Zinfsum)	# division zero error for size = 0
		#sys.exit()

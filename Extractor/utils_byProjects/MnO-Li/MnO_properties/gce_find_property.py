...
rxlist = [ float((i+1)*0.05) for i in range(20) ]
rxlist.insert(0,0.001)

#rxlist = [ 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0 ]

with open(f'PROP_GCE_{_T}.prop','w') as f:
	...
	for _x in rxlist:
		...
		prop_gce_ints = {}
		for prop in proplist:
			prop_gce_ints[prop] = np.float128(0.) 
		
		for size_x, xp in enumerate(npxlist):
		
			for prop in proplist:
		
				prop_gce_ints[prop] += (prop_ce_ints[size_x][prop] * np.exp( xp * _u /_kb/_T ) / _zg)
		
		f.write('%20.12e' % (_x))
		for prop in proplist:
			f.write('%20.12e' % (prop_gce_ints[prop]))
		f.write('\n')

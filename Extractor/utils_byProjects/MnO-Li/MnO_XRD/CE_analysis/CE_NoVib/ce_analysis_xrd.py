...
ttheta = xrd_df[0]['twotheta']
xrd_ce_intslist = []	# for using later
del xrd_df

for size, std_df in enumerate(std_dflist):

	print(f' * processing {size} xrd ...')	

	xrd_df = load_pkl(xrd_pkl_paths[size])
	xrd_ce_ints = [ 0. for i in range(len(ttheta)) ]	# prepare empty ints
	...
	for k, (E,taskid) in enumerate(zip(Elist,taskidlist)):
		...
		xrd_data = xrd_df[taskid]			# get xrd for sample 'taskid'
		xrd_ints = xrd_data['intensity']	# extract intensity for this sample

		E = np.float128(E) - Egm

		# extract individual intensity
		for i, ints in enumerate(xrd_ints):

			xrd_ce_ints[i] += np.float128(ints) * expkbt(E,_T)
	
	xrd_ce_ints_nptmp = np.array(xrd_ce_ints)
	xrd_ce_ints_nptmp = xrd_ce_ints_nptmp * np.exp(-Egm/_kb/_T) / npZlist[size]

	xrd_ce_ints = xrd_ce_ints_nptmp.tolist()
	xrd_ce_intslist.append(xrd_ce_ints)

	print(f' ! writing xrd output ...')

	with open(f'XRD_{_T}_{size}.xrd','w') as f:
		for tt,ints  in zip(ttheta,xrd_ce_ints):

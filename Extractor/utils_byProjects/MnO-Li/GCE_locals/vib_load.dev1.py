...
#csv_files = ['nconp0.csv','nconp1.csv']
#pkl_files = ['freq0.pkl','freq1.pkl']
csv_files = []
pkl_files = []
for s in range(25):
...
for i, (csv,pkl) in enumerate(zip(csv_files,pkl_files)):		# 'i' is for file index !! not structure index
	...
	#for j,row in pkl_dflist[i].iterrows():
	#	print(row)
	...
	for struct_no, taskid in enumerate(taskid_list):
		...
		if not _isdropped:
			...
			#print(f'taskid: {taskid} | ',pkl_df[taskid])
			
	# print(f' * {_drop_cnt}/{len(taskid_list)} structures with imaginary frequencies were found :',imag_freqlist)	# show taskids
...
#xlist = [ np.float128(float(i)/24.) for i in range(25) ]
...
'''
	writing G x
'''
with open(f'vib_Gx{_T}.out','w') as f:
...
#ulist = [ float(i)*_du + _corr for i in range(-_bin,+_rbin+1)]
#print(f'chemical potential u: window [ {-_bin*_du + _corr} : {+_rbin*_du + _corr} ]')
#ulist = [ float(i)*_du for i in range(-_bin,+_rbin+1)]
#print(f'chemical potential u: window [ {-_bin*_du} : {+_rbin*_du} ]')
npulist = np.array(ulist,dtype=np.float128)
...
with open(f'vib_T{_T}_all.out','w') as f:

...
from Transition_ReIm_MPI import get_trans_int_full_mpi # (icube_filelist,fcube_filelist,verbose=False):
...
# USER CONTROL
#
# Example scanning range for transition calculations
# 
# * selected number of HOMO and LUMO states must be equal!
# * you must provide spin-polarised & SOC enabled FHIaims generated CUBE files
#   > each state will be formed of four cube files
#     (1) spin channel 1 Real
#     (2) spin cahnnel 1 Imag
#     (3) spin channel 2 Real
#     (4) spin cahnnel 2 Imag
#
# * if selected HOMO and LUMO ranges range_start - range_end
# > _scan_range parameter parameter is:
#
#   _scan_range = ( (range_end - range_start) -1 ) / 2
# 
#   Examples : 
...
for i in range(homo_states[0],homo_states[-1]+1):
	for f in range(lumo_states[0],lumo_states[-1]+1):
		...
		#transInt, transMoment = get_trans_int_full_p(istate_cubelist,fstate_cubelist)
		#transInt, transMoment = get_trans_int_full_mpi(istate_cubelist,fstate_cubelist,comm)

		if rank == 0:
 		   print(f"{istate:10d}{istate_info['soc_eval']:12.6f}{fstate:10d}{fstate_info['soc_eval']:12.6f}\t",end="",flush=True)
		transInt, transMoment = get_trans_int_full_mpi(istate_cubelist,fstate_cubelist,comm)
		if rank == 0:
			print(f"{transInt:16.9f}\t{transMoment}")
		#if rank == 0:

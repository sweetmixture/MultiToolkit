...
with open('uvlor.txt','w') as f:
    for x,y in zip(lor.keys(),lor.values()):
        f.write(f'{x:20.12e}{y:20.12e}\n')
...
with open('scaled_expuvlor.txt','w') as f:
    for x,y in zip(expx2,expI2):
        f.write(f'{x:20.12e}{y:20.12e}\n')
...
ax.legend(fontsize=_fs-2, loc='upper left', frameon=False)

#ax.set_xlabel('Energy (eV)', fontsize=14)
#ax.set_xlabel('$\it{E}$$_\mathrm{f}$ - $\it{E}$$_\mathrm{i}$ (eV)', fontsize=_fs)
ax.set_xlabel('Photon energy (eV)', fontsize=_fs)

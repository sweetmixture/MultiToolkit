...
def get_trans_int(icube,fcube,verbose=False):
	...
	ispacing = icube_obj['spacing']    # icube grid spacing
	...
	#
	# ADD grid checking / spacing checking : for portability later (writte, 05.2024 - WKJEE)
	#
	...
	#for nx in tqdm(range(nx_max), desc='Integral progress'):
		for ny in range(ny_max):
			for nz in range(nz_max):
				...
				sig = np.linalg.norm(ir * idens * fdens)**2.
				tint += sig

...
csvfile = os.path.join(cwd,f'summary.csv')
...
if mode == '-serial':
	...
	for i,taskmap in enumerate(tasklist):
		...
		rdf_element['SS'] = result[2][0]
		rdf_element['SLi'] = result[2][1]
		rdf_element['SFe'] = result[2][2]
		rdf_element['LiLi'] = result[2][3]
		rdf_element['LiFe'] = result[2][4]
		rdf_element['FeFe'] = result[2][5]
		# --------- USER DEF

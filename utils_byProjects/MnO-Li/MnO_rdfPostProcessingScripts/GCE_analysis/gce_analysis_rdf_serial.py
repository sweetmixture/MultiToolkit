...
#
rdf_ce_ints = [ {} for i in range(len(sizelist)) ]
for i in range(len(rdf_ce_ints)):
	for pair in pairlist:
		rdf_ce_ints[i][pair] = [ 0. for j in range(len(rdf_r)) ]
#
...
for size_x, (std_df,xp) in enumerate(zip(std_dflist,npxlist)):
	...
	# load_rdf 
	rdf_df = load_pkl(rdf_pkl_paths[size_x])
